...
- 默认跟随相机应保持稳定，不要在普通移动时持续根据输入方向偏移，否则容易造成画面漂移和眩晕。
- 方向预看只应在玩家持续按住同一方向，并且角色几乎没有朝该方向移动时触发，表达“被边界、河流、墙体或障碍挡住后向外眺望”。
- 第一次触发受阻眺望应有短暂延迟，例如 `0.3~0.5` 秒，避免玩家轻微碰撞或转向时误触发。
- 一旦已经进入受阻眺望状态，后续输入方向组合变化应连续转向新的受阻方向，不应先回到玩家再重新眺望。例如上方受阻眺望中按下右键，应平滑从上方眺望切到右上眺望。
- 松开所有受阻方向，或玩家重新能朝输入方向移动后，相机应平滑回到玩家身上。
- 玩家移动边界和相机可视边界是两套概念：玩家不能进入河流或地图外，但相机可以在配置的 `Camera2D.limit_*` 范围内轻微看到外侧区域。
- 相机手感参数应暴露为 Inspector 可调属性，并使用中文说明用途、单位和调参影响。
