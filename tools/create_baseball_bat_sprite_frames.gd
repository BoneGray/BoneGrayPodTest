...
const SOURCE_DIR := "res://assets/equipment/weapons/baseball_bat"
const OUTPUT_PATH := "res://resources/equipment/weapons/baseball_bat/baseball_bat_sprite_frames.tres"
...
func _initialize() -> void:
	...
	if error != OK:
		push_error("Failed to save Baseball Bat SpriteFrames: %s" % error)
		quit(1)
		return
...
func _animation_names_from_path(path: String) -> PackedStringArray:
	...
	base_name = base_name.trim_prefix("baseball_bat_")
	...
	var names := _animation_names_from_parts(base_name.split("_", false))
	return names
