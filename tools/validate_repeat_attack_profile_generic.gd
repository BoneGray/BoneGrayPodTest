...
func _run() -> void:
	...
	if not await _validate_tap_combo_animation_finish_clears_cooldown(player, "Unarmed"):
		_fail(root, "Unarmed tap_combo should not stay locked by long cooldown after animation finish.")
		return
	...
	if not await _validate_tap_combo_animation_finish_clears_cooldown(player, "Baseball bat"):
		_fail(root, "Baseball bat tap_combo should not stay locked by long cooldown after animation finish.")
		return
...
func _validate_no_held_repeat(player: CharacterBody2D, label: String) -> bool:
	...
	var repeated_from_hold := float(player.get("attack_cooldown_remaining")) > 0.0
...
func _validate_short_press_cancel(player: CharacterBody2D, label: String) -> bool:
	...
	player.set("attack_cooldown_remaining", 99.0)
	sprite.animation = &"attack_down_first"
	sprite.frame = 0
	player.call("_buffer_primary_attack_input")
	await physics_frame
	if float(player.get("attack_cooldown_remaining")) < 90.0:
...
func _validate_tap_combo_animation_finish_clears_cooldown(player: CharacterBody2D, label: String) -> bool:
	...
	if attack_profile == null:
		push_error("%s is missing an attack profile for cooldown validation." % label)
		return false

	var old_cooldown := float(attack_profile.get("cooldown"))
	attack_profile.set("cooldown", 10.0)
	player.call("_begin_primary_attack")
	await process_frame
	if float(player.get("attack_cooldown_remaining")) < 9.0:
		attack_profile.set("cooldown", old_cooldown)
		push_error("%s did not apply the long cooldown at attack start." % label)
		return false

	player.call("_on_animation_player_finished", &"attack_down_first")
	await process_frame
	var cooldown_after_finish := float(player.get("attack_cooldown_remaining"))
	attack_profile.set("cooldown", old_cooldown)
	if cooldown_after_finish > 0.0:
		push_error("%s stayed cooldown-locked after tap_combo animation finished." % label)
		return false
	return true
