...
const OUTPUT_SCENE_PATH := "res://scenes/characters/enemy_zombie_small.tscn"
...
func _initialize() -> void:
	...
	if base_scene == null or stats == null or sprite_frames == null:
		push_error("Could not load Zombie Small scene inputs.")
		quit(1)
		return

	var root := base_scene.instantiate() as CharacterBody2D
	root.name = "EnemyZombieSmall"
	root.stats = stats
	...
	if pack_error != OK:
		push_error("Failed to pack Zombie Small scene: %s" % pack_error)
		root.queue_free()
		quit(1)
		return
	...
	if save_error != OK:
		push_error("Failed to save Zombie Small scene: %s" % save_error)
	else:
...
func _tune_collision_shapes(root: Node) -> void:
	...
	(body_shape.shape as RectangleShape2D).size = Vector2(8, 10)
	hitbox_shape.position = Vector2(0, 2)
	(hitbox_shape.shape as RectangleShape2D).size = Vector2(8, 14)
	(attack_shape.shape as RectangleShape2D).size = Vector2(16, 12)
...
func _attack_hit_windows_from_frames(sprite_frames: SpriteFrames) -> Dictionary:
	...
	for action_name in ATTACK_HIT_FRAMES.keys():
		var animation_name := "attack_down_%s" % action_name.trim_prefix("attack_")
		var speed := sprite_frames.get_animation_speed(animation_name)
