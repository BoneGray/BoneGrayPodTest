...
const ENEMY_SCRIPT_PATH := "res://scripts/simple_hit_target.gd"
...
func _initialize() -> void:
	...
	if player_sprite_frames == null or enemy_sprite_frames == null or player_script == null or enemy_script == null or camera_script == null:
		push_error("Missing character scene dependency.")
		quit(1)
		return

	if not _save_scene(_create_player_scene(player_sprite_frames, player_script), PLAYER_SCENE_PATH):
		quit(1)
		return

	if not _save_scene(_create_enemy_scene(enemy_sprite_frames, enemy_script), ENEMY_SCENE_PATH):
		quit(1)
		return
...
func _create_player_scene(sprite_frames: SpriteFrames, player_script: Script) -> PackedScene:
	...
	_add_hitbox(player, "player", PLAYER_HITBOX_POSITION, PLAYER_HITBOX_SIZE)
	_add_attack_area(player, PLAYER_ATTACK_AREA_POSITION, PLAYER_ATTACK_AREA_SIZE)
	_add_animation_player(player, sprite_frames, PLAYER_ATTACK_HIT_FRAMES, PLAYER_ATTACK_AREA_OFFSETS)
	return _pack_root(player)


func _create_enemy_scene(sprite_frames: SpriteFrames, enemy_script: Script) -> PackedScene:
	...
	_add_hitbox(enemy, "enemy", ENEMY_HITBOX_POSITION, ENEMY_HITBOX_SIZE)
	_add_attack_area(enemy, ENEMY_ATTACK_AREA_POSITION, ENEMY_ATTACK_AREA_SIZE)
...
func _add_attack_area(root: Node, area_position: Vector2, area_size: Vector2) -> void:
...
func _add_hitbox(root: Node, group_name: String, shape_position: Vector2, shape_size: Vector2) -> void:
	var hitbox := Area2D.new()
	hitbox.name = "HitboxArea2D"
	hitbox.add_to_group(group_name, true)
